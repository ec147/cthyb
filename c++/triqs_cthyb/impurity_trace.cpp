...
namespace triqs_cthyb {
  ...
  std::pair<int, matrix_t> impurity_trace::compute_matrix(node n, int b) {
    ...
    if (n->right) { // M <- M * exp * r[b]
      ...
      if (updating) {    
        if (n->cache.exp_r[b1].size() != dim) n->cache.exp_r[b1] = std::vector<double>(dim);
        for (int i = 0; i < dim; ++i) {
    ...
    if (n->left) { // M <- l[b] * exp * M
      ...
      if (updating) { 
        if (n->cache.exp_l[b2].size() != dim) n->cache.exp_l[b2] = std::vector<double>(dim);
        for (int i = 0; i < dim; ++i) {
  ...
  void impurity_trace::compute_matrix_left(node n, int b, matrix_t &Mleft, bool is_empty, double dtau_beta) {
    ...
    dimb1 = get_block_dim(b1);
    dimb2 = get_block_dim(b2);
  ...
  std::pair<h_scalar_t, h_scalar_t> impurity_trace::compute(double p_yee, double u_yee, bool meas_den) {  
    ...
    for (bl = 0; bl < n_bl; ++bl) { // sum over all blocks
      ...
      if (use_norm_as_weight || (meas_den && !time_invariance)) {
